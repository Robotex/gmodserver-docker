...
/usr/games/gameserver/serverfiles/srcds_run -game $SRCDS_GAME -ip $BIND_IP -port $BIND_PORT +clientport $SRCDS_CLIENT_PORT +host_workshop_collection $WORKSHOP_COLLECTION -authkey $AUTHKEY -pingboost 1 -timeout 0 -tvdisable -nohltv -strictportbind +exec server/autoexec.cfg "$@" -norestart -steam_dir ~/.steam/steamcmd -steamcmd_script /usr/games/gameserver/update.txt +hostname \"${SRCDS_HOSTNAME}\" &
