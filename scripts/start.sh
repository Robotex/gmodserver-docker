...
/usr/games/gameserver/serverfiles/srcds_run -game $SRCDS_GAME -ip $BIND_IP -port $BIND_PORT +clientport $SRCDS_CLIENT_PORT +host_workshop_collection $WORKSHOP_COLLECTION -authkey $AUTHKEY +r_hunkalloclightmaps 0 -pingboost 2 -timeout 0 -tvdisable -nohltv -strictportbind "$@" -norestart -steam_dir ~/.steam/steamcmd -steamcmd_script /usr/games/gameserver/update.txt +hostname \"${SRCDS_HOSTNAME}\" &
