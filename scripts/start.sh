...
/usr/games/gameserver/serverfiles/srcds_run -game $SRCDS_GAME -ip $BIND_IP -port $BIND_PORT +host_workshop_collection $WORKSHOP_COLLECTION -authkey $AUTHKEY +r_hunkalloclightmaps 0 -pingboost 2 -timeout 0 -tvdisable -strictportbind "$@" -norestart -steam_dir ~/.steam/steamcmd -steamcmd_script /usr/games/gameserver/update.txt +hostname \"${SRCDS_HOSTNAME}\" &
